...
final class TradeViewModel : ObservableObject{
    ...
    func getall(){
        ...
        let clothesURL = vm.HOST_URL+"outfit/allOFT"
    ...
    func getswiped(completed: @escaping (Bool) -> Void)  {
        ...
        print(useridd ?? "noope")
        let clothesURL = vm.HOST_URL+"outfit/getswiped/"
        if let url = URL(string:clothesURL) {
            URLSession
                .shared
                .dataTask(with: url) { [weak self] data, response, error in
    ...
    func swipeleft(IdOutfit: String, IdOutfitR: String , reciver: String) {
        print(reciver)
        AF.request(vm.HOST_URL+"match/swipe/"+reciver,
                   method: .put,
                   parameters: [
                    "IdOutfit": IdOutfit,
                    "IdOutfitR": IdOutfitR,
            ...
            case .success:
              ...
              //  print(jsonData["userID"])
                ...
                if jsonData["firebase"].exists() {
                    ...
                    let user = jsonData["userID"].description
